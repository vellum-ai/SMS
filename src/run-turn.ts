 ...
 * delivery reaches this plugin it has already been verified, gated, deduped:
 * the kill switch, the trust verdict, the identity canonicalization and the
 * verification and invite intercepts have all had their say, and the admission
 * floor has been compared against the sender's trust. Nothing that could
 * refuse this message is left, which is what makes it safe to act on one the
 * moment it arrives.
 *
 * What remains is the part only a plugin can do. The assistant has no adapter
 * that speaks to Twilio, so the reply goes back out over the same provider the
 * delivery came in on, using credentials the gateway never holds.
 */
 ...
 *
 * Declared here rather than imported because `@vellumai/plugin-api` may not
 ...
 * webhook response is not a delivery channel: Twilio answers a webhook with an
 * acknowledgement, not with a message to forward. The turn's answer has to go
 * out over the provider's own send API, addressed to the number the delivery
 * came from.
 *
...
export async function runTurnForDelivery(opts: {
  ...
  provider: import("./providers/types.ts").MessagingProvider;
