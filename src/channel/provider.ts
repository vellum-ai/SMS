...
export function buildChannelProvider(
  ...
  return {
    ...
    normalize(raw: unknown, receivedAt: string): PluginInboundEvent | undefined {
      // The host contract asks for an optional event, so the verdict is
      // collapsed here rather than at the provider seam. The webhook route
      // keeps the delivery kinds apart, which is where the difference is
      // worth something.
