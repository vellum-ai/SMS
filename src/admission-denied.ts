 ...
 *
 * The gateway keeps the vendor webhook away from this plugin on a floor deny,
 * because that path is free to run a turn. Only this plugin can send on the
 * Twilio line, so the gateway posts a structured notice here instead. The
 * reply is the same line built-in channels send. There is no conversation
 * turn.
 */
...
import { TWILIO_INGRESS_ROUTE_PATH } from "./webhook-endpoint.ts";
 ...
 *
 * The ingress route the notice names has to match this plugin's only route —
 * a notice naming anything else is not this channel's business, and retrying
...
export async function handleAdmissionDeniedNotice(
  ...
  if (notice.data.ingressRoute !== TWILIO_INGRESS_ROUTE_PATH) {
    return json(200, { ok: true, ignored: "notice is not for this route" });
  }
