 ...
 * set` is the only way in — the same CLI the `sms` skill's script already
 ...
 *
 * Reported for all providers rather than just the configured one, so a future
 * second provider does not need a round trip to find out whether it is ready.
 */
 ...
 * "command failed with exit code 1" tells a user nothing about which of three
 * Twilio fields did not take.
 */
